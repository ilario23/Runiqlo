...
const TYPE_CONFIG: Record<WorkoutType, {label: string; color: string; icon: LucideIcon}> = {
  ...
  rest:         {label: 'Rest',         color: 'text-[var(--faint)] border-[var(--line)] bg-[var(--bg)]/4',                     icon: Moon},
  ...
  return (
    ...
    >
      ...
      {workout.completed && workout.linkedStravaActivityId && (
        <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-0.5">
          ...
          <a
            ...
            className="flex shrink-0 items-center gap-1 text-xs text-[var(--faint)] hover:text-brand hover:underline"
          >
      ...
      {!workout.completed && isToday && onMarkDone && (
        <button
          onClick={onMarkDone}
          className="mt-1.5 text-xs text-[var(--dim)] hover:text-[var(--text)] underline transition-colors"
        >
          Mark complete
        </button>
      )}
