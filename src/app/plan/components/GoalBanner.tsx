  ...
  return (
    ...
    >
      ...
      {goal.targetEventName && (
        <span className="text-xs text-[var(--faint)] truncate max-w-[120px]">
          {goal.targetEventName}
        </span>
      )}
      ...
      {currentPhase ? (
        <div className="flex items-center gap-1.5">
          <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${PHASE_DOT[currentPhase] ?? 'bg-[var(--bg)]/30'}`} />
          <span className={`text-[10px] font-semibold uppercase tracking-widest ${PHASE_TEXT[currentPhase] ?? 'text-[var(--faint)]'}`}>
            {currentPhase}
          </span>
        </div>
      ) : (
        <span className="text-[10px] text-[var(--faint)] uppercase tracking-wider">No plan</span>
      )}
      ...
      {/* Weeks away */}
      {weeksAway != null && (
        <span className={`text-xs tabular-nums ${weeksAway > 0 ? 'text-[var(--faint)]' : 'text-[var(--color-accent)]'}`}>
      ...
      {goal.experienceLevel && (
        <span className="ml-auto text-[10px] text-[var(--faint)] uppercase tracking-wider hidden md:block">
          {goal.experienceLevel}
        </span>
      )}
    </div>
